    ...
    return (
        <div className="min-h-screen bg-slate-50">
            ...
            <main className="p-4 lg:p-6 max-w-7xl mx-auto">
                    ...
                    <div className="flex items-center justify-between gap-3 mb-1">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                            <a href="/products/export"
                                className="h-9 px-3 flex items-center gap-1.5 bg-white border border-slate-200 hover:bg-white/80 text-slate-600 hover:text-slate-800 rounded-xl text-sm font-medium transition-all whitespace-nowrap">
                                <IconDownload className="w-4 h-4" /> CSV
                            </a>
                            <Link href="/products/create"
                                className="h-9 px-3 flex items-center justify-center bg-emerald-700 hover:bg-emerald-800 text-white font-bold rounded-xl text-sm transition-colors whitespace-nowrap">
                                + Nouveau
                            </Link>
                        </div>
                    </div>
                ...
                {/* Flash success */}
                {flash?.success && (
                    <div className="mb-5 p-4 bg-emerald-50/80 border border-emerald-200/60 rounded-xl text-emerald-700 text-sm flex items-center gap-2">
                        ...
                        {products.map((product) => {
                            ...
                            return (
                                <div key={product.id_produit}
                                    className="bg-white/70 rounded-lg border border-slate-200 overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all shadow-sm group">
                                    ...
                                    <Link href={`/products/${product.id_produit}`}
                                        ...
                                        {/* Badge stock */}
                                        <span className={`absolute top-3 right-3 text-xs font-bold px-2.5 py-1 rounded-full flex items-center gap-1 ${stockBas ? 'bg-white/30 text-white' : 'bg-white/90 text-emerald-600'}`}>
                                    ...
                                    <div className="p-4">
                                        <Link href={`/products/${product.id_produit}`} className="block mb-3">
                                            <h3 className="font-bold text-slate-800 hover:text-emerald-600 transition-colors text-lg truncate">
                                                {product.nom}
                                            </h3>
                                        ...
                                        <div className="flex gap-2">
                                            <Link href={`/products/${product.id_produit}/edit`}
                                                className="flex-1 h-10 flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-600 hover:text-emerald-600 rounded-lg text-sm font-medium transition-all">
                                                <IconEdit className="w-4 h-4" /> Modifier
                                            </Link>
                                            <button onClick={() => handleDelete(product.id_produit, product.nom)}
                                                className="flex-1 h-10 flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 text-emerald-600 hover:text-emerald-700 rounded-lg text-sm font-medium transition-all">
