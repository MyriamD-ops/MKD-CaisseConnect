...
function PieChart({ data, total }) {
    if (!data || data.length === 0 || total === 0)
        return <div className="flex items-center justify-center h-40 text-slate-500 text-sm">Aucune donnée</div>;
            ...
            <main className="p-4 lg:p-6 max-w-5xl mx-auto">
                ...
                {periodeActive === 'evenement' && (
                    <div className="mb-6 no-print">
                        ...
                        {evSel && <p className="text-xs text-slate-500 mt-2 ml-1">Toutes les ventes entre le {evSel.date_debut} et le {evSel.date_fin}</p>}
                    </div>
                )}
                ...
                <p className="text-center text-xs text-slate-400 mt-6">Données en temps réel · MKD CaisseConnect</p>
            </main>
        </div>
    );
}
